...
**For a normal population, mean unknown, and known standard deviation**
...
* Confidence interval: $\bar x - z_{\alpha/2} {s \over \sqrt{n}} \le \mu \le \bar x + z_{\alpha/2} {s \over \sqrt{n}}$
* Margin of  error:
...
* Confidence Interval: $\bar x-z_{\alpha/2} \cdot \sigma/{\sqrt n} \le \mu \le \bar x + z_{\alpha/2} \cdot  \sigma/{\sqrt n}$
* Margin of Error
```{r eval=FALSE}
me <- qnorm(alpha/2,lower.tail=FALSE)*sigma/sqrt(n)
```
...
* Tails: $\chi^2_{\alpha/2,n-1}$ (upper) and $\chi^2_{\alpha/2,n-1}$ (lower) $\chi^2$ tails of the $\chi^2$ distribution.
* Confidence Interval: 
  $${(n-1)s^2 \over \chi^2_{\alpha/2,n-1} } \le \sigma^2 \le {(n-1)s^2 \over \chi^2_{1-\alpha/2,n-1} }$$
